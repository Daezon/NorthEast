...
exports.getbooking = async (req,res) =>{
    try{   
        ...
        const carturl = await axios.get(`http://localhost:8080/api/cart/${Clientid}`);
        ...
        console.log(findDate)
          if(findDate.length >= 1){
            return res.status(403).send({ message: 'Scheduled time is Already Taken!' });
          }else{
...
exports.updateCanBooking = async(req,res) => {
    try{
        ...
        return res.status(200).json({ data: updateBooking, message: 'Booking Updated', status: 200 });
    }catch(err){
        return res.status(400).json({ message: err.message, status: 400 });
    }
};
